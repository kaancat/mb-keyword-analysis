...
class SearchConsoleService:
    ...
    def _find_matching_site(self, url):
        ...
        try:
            ...
            clean_input = url.replace("https://", "").replace("http://", "").replace("www.", "").strip("/")
            
            print(f"--- GSC Lookup: Searching for '{clean_input}' in {len(sites)} sites ---")
            
            # 3. Search for Match
            for site in sites:
                # Normalize Site ID
                clean_site = site.replace("sc-domain:", "").replace("https://", "").replace("http://", "").replace("www.", "").strip("/")
    ...
    def get_organic_performance(self, site_url, days=30):
        ...
        if not actual_site_url:
            return {"error": f"Could not find GSC property for {site_url}. Ensure you have access and the URL is correct."}
            
        end_date = datetime.now().strftime("%Y-%m-%d")
        start_date = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
        
        print(f"--- Fetching GSC Data for: {actual_site_url} ({start_date} to {end_date}) ---")
